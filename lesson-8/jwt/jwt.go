...
var (
	JWTHeadB64  = []byte("e1wiYWxnXCI6IFwiSFMyNTZcIixcInR5cFwiOiBcIkpXVFwifQo=")
	JWTHead     = "{\"alg\": \"HS256\",\"typ\": \"JWT\"}"
	SecretBytes = []byte("secret-key")
	SepBytes    = []byte(".")
)
...
func buildSignBytes(head, payload []byte) ([]byte, error) {
	// Подготовим хэш функцию для подписи
	// Она будет основана на алгоритме sha256
	// Ключ для простоты будем хранить в константе
	...
	// конкатенируем через точку head и payload
	// эти данные мы будем подписывать
	_, err := hasher.Write(bytes.Join([][]byte{head, payload}, SecretBytes))
	if err != nil {
		return nil, err
	}
	...
	res := make([]byte, enc.EncodedLen(len(signBin)))
	base64.RawURLEncoding.Encode(res, signBin)
	return res, nil
}

func Make(data Payload) (string, error) {

	// Сначала сериализуем Payload
...
func MakeFaster(data PayloadFaster) (string, error) {
	...
	// Формируем подпись токена
	sign, err := buildSignBytes(JWTHeadB64, dataB64)
	if err != nil {
		return "", fmt.Errorf("Build sign error: %w", err)
	}

	// конкатенируем итоговый результат
	return string(bytes.Join([][]byte{JWTHeadB64, dataB64, sign}, SepBytes)), nil
}
...
// func MakeFast(data Payload) (string, error) {
